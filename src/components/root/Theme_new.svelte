<script context="module" lang="ts">
	...
	function initDarkModeFromCss(node: HTMLButtonElement) {
		// STEP 2: initialize state from css
		isDarkMode = document.documentElement.classList.contains('dark');

		// handle changes in darkmode
		...
		return {
			destroy() {
				...
				node.removeEventListener('click', toggleDarkMode);
			}
		};
	}
</script>
...
<button class="fadeIn w-[24px]" use:initDarkModeFromCss>
	{#key isDarkMode}
		<img
