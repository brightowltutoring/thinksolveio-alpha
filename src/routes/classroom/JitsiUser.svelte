<script lang="ts">
	...
	let isDarkMode = getContext<() => boolean>('isDarkMode');
	const setDarkMode = getContext<(p: boolean) => void>('setDarkMode');
	...
	onMount(() => {
		...
		return () => initiallyFromLightMode && setDarkMode(false);
	});
...
<style lang="postcss">
	/* #meet {
		@apply relative h-[90vh] w-full -translate-y-36 md:h-[670px] md:-translate-y-10 pwa:h-[85vh];
	} */
	#meet {
